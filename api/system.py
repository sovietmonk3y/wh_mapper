...
class SystemNameAutocompleteApi(View):
    ...
    def get(self, request, name_portion):
        if name_portion.strip() == '':
            return HttpResponseBadRequest('Invalid text supplied for autocompletion')

        systems = wh_mapper_models.System.objects.filter(name__istartswith=name_portion).values_list('name', flat=True)[:SYSTEM_NAME_AUTOCOMPLETE_MAX_RESULTS]
